...
class BricksDisplay < Widget
    ...
    def start_level
        level_config_file_name = File.join(File.dirname(File.dirname(__FILE__)), 'maps/maps/', "level#{@level}.txt")
...
class GameMessageOverlay < InfoBox
    def initialize(title, content_id)
        content_file_name = File.join(File.dirname(File.dirname(__FILE__)), 'msgs', "messages_#{content_id}.txt")
